...
function astroride_body_classes( $classes ) {
	...
	if ( astroride_image_filters_enabled() ) {
 ...
 */
function astroride_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'astroride_comment_form_defaults' );

/**
 * Filters the default archive titles.
 */
function astroride_get_the_archive_title() {
	if ( is_category() ) {
		$title = __( 'Category Archives: ', 'astroride' ) . '<span class="page-description">' . single_term_title( '', false ) . '</span>';
	} elseif ( is_tag() ) {
		$title = __( 'Tag Archives: ', 'astroride' ) . '<span class="page-description">' . single_term_title( '', false ) . '</span>';
	} elseif ( is_author() ) {
		$title = __( 'Author Archives: ', 'astroride' ) . '<span class="page-description">' . get_the_author_meta( 'display_name' ) . '</span>';
	} elseif ( is_year() ) {
		$title = __( 'Yearly Archives: ', 'astroride' ) . '<span class="page-description">' . get_the_date( _x( 'Y', 'yearly archives date format', 'astroride' ) ) . '</span>';
	} elseif ( is_month() ) {
		$title = __( 'Monthly Archives: ', 'astroride' ) . '<span class="page-description">' . get_the_date( _x( 'F Y', 'monthly archives date format', 'astroride' ) ) . '</span>';
	} elseif ( is_day() ) {
		$title = __( 'Daily Archives: ', 'astroride' ) . '<span class="page-description">' . get_the_date() . '</span>';
	} elseif ( is_post_type_archive() ) {
		$title = __( 'Post Type Archives: ', 'astroride' ) . '<span class="page-description">' . post_type_archive_title( '', false ) . '</span>';
	} elseif ( is_tax() ) {
		$tax = get_taxonomy( get_queried_object()->taxonomy );
		/* translators: %s: Taxonomy singular name */
		$title = sprintf( esc_html__( '%s Archives:', 'astroride' ), $tax->labels->singular_name );
	} else {
		$title = __( 'Archives:', 'astroride' );
	}
	return $title;
}
add_filter( 'get_the_archive_title', 'astroride_get_the_archive_title' );

/**
 * Determines if post thumbnail can be displayed.
 */
function astroride_can_show_post_thumbnail() {
	return apply_filters( 'astroride_can_show_post_thumbnail', ! post_password_required() && ! is_attachment() && has_post_thumbnail() );
}

/**
 * Returns true if image filters are enabled on the theme options.
 */
function astroride_image_filters_enabled() {
 ...
 */
function astroride_post_thumbnail_sizes_attr( $attr ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'astroride_post_thumbnail_sizes_attr', 10, 1 );

/**
 * Returns the size for avatars used in the theme.
 */
function astroride_get_avatar_size() {
	return 60;
}
 ...
 */
function astroride_get_discussion_data() {
 ...
 */
function astroride_add_ellipses_to_nav( $nav_menu, $args ) {
	...
	if ( 'menu-1' === $args->theme_location ) :
		...
		$nav_menu .= '<span class="screen-reader-text">' . esc_html__( 'More', 'astroride' ) . '</span>';
		$nav_menu .= astroride_get_icon_svg( 'arrow_drop_down_ellipsis' );
		$nav_menu .= '</button>';
		...
		$nav_menu .= astroride_get_icon_svg( 'chevron_left' );
		$nav_menu .= esc_html__( 'Back', 'astroride' );
		$nav_menu .= '</button>';
		$nav_menu .= '</li>';
		$nav_menu .= '</ul>';
		$nav_menu .= '</li>';
...
add_filter( 'wp_nav_menu', 'astroride_add_ellipses_to_nav', 10, 2 );
 ...
 */
function astroride_nav_menu_link_attributes( $atts, $item, $args, $depth ) {
...
add_filter( 'nav_menu_link_attributes', 'astroride_nav_menu_link_attributes', 10, 4 );
...
function astroride_add_dropdown_icons( $output, $item, $depth, $args ) {
	...
	if ( in_array( 'mobile-parent-nav-menu-item', $item->classes, true ) && isset( $item->original_id ) ) {
		...
		$link = sprintf(
			...
			astroride_get_icon_svg( 'chevron_left', 24 )
		);
		...
		$icon = astroride_get_icon_svg( 'keyboard_arrow_down', 24 );
 ...
 */
function astroride_add_mobile_parent_nav_menu_items( $sorted_menu_items, $args ) {
	static $pseudo_id = 0;
	if ( ! isset( $args->theme_location ) || 'menu-1' !== $args->theme_location ) {
...
add_filter( 'wp_nav_menu_objects', 'astroride_add_mobile_parent_nav_menu_items', 10, 2 );

/**
 * Convert HSL to HEX colors
 */
function astroride_hsl_hex( $h, $s, $l, $to_hex = true ) {
